...
//bfef45
  ...
  svg
    ...
    for (let i = colors.length - 1; i >= 0; i--) {
      svg.append("circle").attr("cx",850).attr("cy",firstCirclePos).attr("r", 6).style("fill", colors[i]);
      svg.append("text").attr("x", 870).attr("y", firstCirclePos).text(keys[i]).style("font-size", "10px").attr("alignment-baseline","middle");
